...
final readonly class ReferenceCodeHttpService
{
    public function __construct(
        ...
        private ModuleAvailabilityService $availability,
    ) {}
        ...
        $response = $this->host->read(
            ...
            function (
                ...
                try {
                    ...
                    $visible = $this->visibleDefinitionRegistry(
                        $modules,
                        $context,
                    ...
                    return new ExternalOperationResponse(200, [
                        'data' => ['items' => $this->query->sets($visible)],
                    ]);
    ...
    public function listCodes(
        ...
        $response = $this->host->read(
            ...
            function (
                AuthorizedExternalOperation $authorized,
                ExternalOperationRequest $query,
            ) use ($modules, $moduleKey, $setKey): ExternalOperationResponse {
                try {
                    $context = self::tenantContext($authorized);
                    self::assertEmptyBody(self::requestPayload($query));
                    $input = self::listQuery(self::requestQuery($query));
                    $definition = self::definitionRegistry($modules)->require($moduleKey, $setKey);
                    $this->assertOwnerAvailable($definition, $context, $query->comparisonTime);
    ...
    public function getCode(
        ...
        $response = $this->host->read(
            ...
            function (
                AuthorizedExternalOperation $authorized,
                ExternalOperationRequest $query,
            ) use ($modules, $moduleKey, $setKey, $code): ExternalOperationResponse {
                try {
                    $context = self::tenantContext($authorized);
                    self::assertEmptyBody(self::requestPayload($query));
                    ...
                    $definition = self::definitionRegistry($modules)->require($moduleKey, $setKey);
                    $this->assertOwnerAvailable($definition, $context, $query->comparisonTime);
                    $entry = $this->query->get($definition, $context, $code, $asOf);
        ...
        $response = $this->host->command(
            ...
            function (
                ...
                try {
                    ...
                    $definition = self::definitionRegistry($modules)->require($moduleKey, $setKey);
                    $this->assertOwnerAvailable($definition, $context, $command->comparisonTime);
                    $entry = $this->admin->create(
                        $definition,
                        $context,
                        $input['code'],
                        $input['label'],
            ...
            guard: $this->commandGuard($moduleKey, $setKey),
        );
    ...
    private function commandGuard(string $moduleKey, string $setKey): callable
    {
        return function (
            AuthorizedExternalOperation $authorized,
            ExternalOperationRequest $request,
        ) use ($moduleKey, $setKey): void {
            try {
                $context = self::tenantContext($authorized);
                $definition = self::definitionRegistry($this->modules)->require($moduleKey, $setKey);
                $this->store->assertCurrentDefinition($definition, true);
    ...
    private function assertModuleAvailable(
        string $moduleKey,
        TenantContext $context,
        DateTimeImmutable $comparisonTime,
        bool $lockAvailabilityReads = false,
    ...
    private static function lockModuleAvailability(int $tenantId, string $moduleKey): void
    {
        Db::name('module_installation')->where('module_key', $moduleKey)->lock('FOR SHARE')->value('module_key');
        Db::name('tenant_module')->where('tenant_id', $tenantId)->where('module_key', $moduleKey)
            ->lock('FOR SHARE')->value('module_key');
    }

    private function visibleDefinitionRegistry(
        ...
        foreach ($declared->moduleKeys() as $moduleKey) {
            ...
            if ($definitions !== [] && !$this->ownerAvailable($definitions[0], $context, $comparisonTime)) {
                continue;
            }
    ...
    private function ownerAvailable(
        ...
        try {
            $this->assertOwnerAvailable($definition, $context, $comparisonTime);
