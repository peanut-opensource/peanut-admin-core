...
final class FileDeliveryHttpRuntime
{
    public static function assets(Request $request, PDOConnection $connection): Response
    {
        ...
        $response = TenantModuleRuntime::host($connection, $modules)->read($op, $external, static function ($authorized, $query) use ($connection) {
            try {
                ...
                $result = (new FileDeliveryStore($connection))->assets($context->tenantId, TenantModuleRuntime::positiveInt($raw['page'] ?? '1', 10000), TenantModuleRuntime::positiveInt($raw['page_size'] ?? '20', 100));
                $items = [];
                foreach ($result['items'] as $item) {
                    $file = (new FileStore($connection))->get($context->tenantId, (string) $item['file_key']);
                    $grant = self::service($connection, $context->tenantId)->issue(new DeliveryRequest($context, $file, DeliveryVisibility::Private, ReplayMode::SingleUse, $query->comparisonTime, 120, true));
    ...
    public static function grant(Request $request, string $fileKey, PDOConnection $connection): Response
    {
        ...
        $response = TenantModuleRuntime::host($connection, $modules)->command($op, $external, static function ($authorized, $command) use ($connection, $fileKey) {
            try {
                self::noInput($command->body);
                $context = TenantModuleRuntime::context($authorized);
                $file = (new FileStore($connection))->get($context->tenantId, $fileKey);
                $grant = self::service($connection, $context->tenantId)->issue(new DeliveryRequest($context, $file, DeliveryVisibility::Private, ReplayMode::SingleUse, $command->comparisonTime, 120, true));
    ...
    public static function deliver(Request $request, string $fileKey, PDOConnection $connection): Response
    {
        ...
        try {
            ...
            [$file, $content] = (new ThinkPhpTransactionManager($connection))->run(
                static function () use ($connection, $pdo, $tenantId, $query, $fileKey, $now, $requestId): array {
                    ...
                    self::tokens($connection, $tenantId)->verifyAndConsume($query['token'], $tenantId, $fileKey, $now);
                    $file = (new FileStore($connection))->getForDownload($tenantId, $fileKey);
    ...
    private static function service(PDOConnection $connection, int $tenantId): DeliveryService
    {
        return new DeliveryService(self::adapter($connection, $tenantId), new DeliveryPolicy());
    }
    private static function adapter(PDOConnection $connection, int $tenantId): LocalSignedDeliveryAdapter
    {
        $c = self::config();
        if ($c['provider'] !== 'local-private' || $c['delivery_adapter'] !== 'local-signed') {
            throw FileMediaException::deliveryUnavailable();
        }return new LocalSignedDeliveryAdapter($c['delivery_base_url'], self::tokens($connection, $tenantId));
    }
    private static function tokens(PDOConnection $connection, int $tenantId): SignedDeliveryTokenService
    {
