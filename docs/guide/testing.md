...
Qualification resources are selected from the registered project resources and
the [release qualification binding](release-qualification.md) at the exact
source tag; one candidate's environment, evidence or result never carries
forward to another. Fixed paths or shared outputs require a serial exclusive
lease, verified absence before use and proven cleanup.
