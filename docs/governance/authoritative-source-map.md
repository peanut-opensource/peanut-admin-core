...
| Product, Core, Edition, Module and customer Instance versions | Application-owned `repo://peanut-admin/docs/architecture/product-version-identity-adr.md`, exact manifests, immutable tags and consumer locks | [Core version projection](../architecture/product-version-identity.md), architecture and release guides; matching numbers do not prove qualification or deployment |
...
| Application/Core PHP Runtime direction | `repo://peanut-admin/docs/architecture/core-thinkphp-runtime-direction-adr.md`, current Core/Application source and tests, fixed two-repository source snapshots and the accepted product version identity | [Core architecture projection](../architecture/index.md#accepted-thinkphp-8-runtime-direction), Module guide and domain package references; development implementation does not change published package identity or qualification |
...
| Historical publication | immutable source/split tags, Registry metadata and the matching committed qualification record | status index and archived evidence retain their original time scope |
