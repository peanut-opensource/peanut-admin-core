...
Kernel Tenant and Platform Authorization now evaluate principal, role, Module
and permission state entirely through their existing ThinkPHP Models and ORM
...
Tenant owner candidate provisioning now uses the existing account, credential,
member/role, Tenant, operator and platform-audit Models while preserving the
single-candidate lock, activation idempotency lookup and dual-audience audit.
...
`61546084e1e07f1c41df8d2383dbbe1d77a83b16`:
...
| Reference host and starter | 106 production files reference ThinkPHP while 89 still reference PDO; 12 RuntimeFactory classes and 19 concrete ModuleProviders hand-assemble much of the PDO graph. HTTP enters ThinkPHP, but installation, upgrade, health and worker paths have not yet converged on one data/bootstrap path. |
