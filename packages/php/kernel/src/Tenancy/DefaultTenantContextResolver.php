...
final readonly class DefaultTenantContextResolver
{
    ...
    public function system(string $actor, string $operation, string $operationId): TenantSystemContext
    {
        ...
        if ($actor === '' || $operation === '' || $operationId === '') throw new \DomainException('DEFAULT_TENANT_CONTEXT_UNAVAILABLE');
        $statement = $this->pdo->prepare("SELECT id FROM pa_tenant WHERE code = 'default' AND status = 'active' LIMIT 2");
        $statement->execute();
        $ids = $statement->fetchAll(PDO::FETCH_COLUMN);
        if (count($ids) !== 1 || (int) $ids[0] < 1) throw new \DomainException('DEFAULT_TENANT_CONTEXT_UNAVAILABLE');
    ...
    {
        if (!method_exists($request, 'header')) throw new \DomainException('DEFAULT_TENANT_CONTEXT_UNAVAILABLE');
